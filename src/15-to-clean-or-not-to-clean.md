...
As I've been using throughout the course of this article, I will once again use CPU's to compare. This is the equivelent of going from a 
- Intel i5 14600K
  - Released October 2023
- Intel i5 11600K
  - Released March 2021
...
- Easier to see when async Tasks are unneeded (e.g. removing MediatR, was forcing use of Task)
...
You may remember earlier in the post when I mentioned about how one of the experiments I tried failed. For transparency the idea I tried was moving some of the services into [Singletons](https://en.wikipedia.org/wiki/Singleton_pattern). 

My thinking here was, if we don't need to keep allocating and freeing memeory, we may be able to get a speedup by not having to ask the hardware to do so much. My theory was incorrect and so the figures went from `160.4ms` average to `167.4ms` on average which equates to about a 4% slowdown from making the switch. 
